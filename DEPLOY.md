...
- **Backend:** funciones serverless en la carpeta `api/` (Vercel las detecta automáticamente):
  ...
  - `api/chat.js` — responde con **GPT-4o-mini** (OpenAI) usando el historial como contexto y guarda cada mensaje.
...
2. Verifica que tu cuenta tenga acceso al modelo `gpt-4o-mini`.
...
5. **La IA responde un error de OpenAI** → el chat ahora muestra el mensaje exacto:
   ...
   - `model_not_found` → tu cuenta aún no tiene acceso a `gpt-4o-mini`.
...
api/                ← funciones serverless (Node 20)
  ...
  chat.js           ← POST  /api/chat       → GPT-4o-mini + historial
